...
class CritCallbackIt: public RBOptimizable
  {
  public:
    explicit CritCallbackIt(ContinuousModelIt* model):mBO(model){};
    double evaluate(const vectord &query) 
    {
      return mBO->evaluateCriteria(query);
    }
  private:
    ContinuousModelIt* mBO;
  };
   
  ContinuousModelIt::ContinuousModelIt(size_t dim, Parameters parameters):
    BayesOptBaseIt(dim,parameters)
  { 
    mCallback.reset(new CritCallbackIt(this));
  ...
  ContinuousModelIt::~ContinuousModelIt()
  {
    //    delete cOptimizer;
  } // Default destructor

  void ContinuousModelIt::setBoundingBox(const vectord &lowerBound,
  ...
  //////////////////////////////////////////////////////////////////////
   ...
   vectord ContinuousModelIt::samplePoint()
  {	    
  ...
  void ContinuousModelIt::findOptimal(vectord &xOpt)
  { 
  ...
  vectord ContinuousModelIt::remapPoint(const vectord& x)
  {
